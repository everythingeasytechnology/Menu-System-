...
Service point responses include live occupancy: if a point has active orders (`preparing`, `ready`, or `served`), `status` is returned as `occupied` with `active_order_count` and `active_orders`. The `status=occupied` list filter also includes points occupied because of active orders.

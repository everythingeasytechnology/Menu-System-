...
@php
    $metricCards = [
        [
            ...
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.9"><path stroke-linecap="round" stroke-linejoin="round" d="M7 7.25h10M7 11.75h10M7 16.25h6M5.75 4.75h12.5v14.5H5.75V4.75Z" /></svg>',
        ],
        [
            'label' => "Today's Sales",
        ...
        [
            ...
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.9"><path stroke-linecap="round" stroke-linejoin="round" d="M5.25 8.25h13.5v9.5H5.25v-9.5Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M8.5 6h7M8.5 12h4.5" /></svg>',
        ],
        [
            'label' => 'Completed Orders',
            'value' => number_format($stats['completed']),
            'box' => 'bg-blue-100 text-blue-600',
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.9"><path stroke-linecap="round" stroke-linejoin="round" d="M7.75 12.5 10.5 15.25 16.75 8.75" /><path stroke-linecap="round" stroke-linejoin="round" d="M12 20.25a8.25 8.25 0 1 0 0-16.5 8.25 8.25 0 0 0 0 16.5Z" /></svg>',
        ],
        [
            'label' => 'Open Orders',
            'value' => number_format($stats['pending']),
            'box' => 'bg-warning/10 text-warning',
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.9"><path stroke-linecap="round" stroke-linejoin="round" d="M12 7.25v5l3 2" /><path stroke-linecap="round" stroke-linejoin="round" d="M12 20.25a8.25 8.25 0 1 0 0-16.5 8.25 8.25 0 0 0 0 16.5Z" /></svg>',
        ],
    ];
...
>
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-black tracking-tight text-ink">Dashboard</h1>
            <p class="mt-0.5 text-xs font-semibold text-muted">Overview of your restaurant operations for {{ $business->name }}.</p>
        </div>
        <div class="flex items-center gap-2.5">
            <button
                type="button"
                @click="refreshOrders()"
                class="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-border bg-card px-4 text-xs font-black text-ink shadow-sm transition hover:bg-card-tint"
            >
            ...
            <a
                ...
                class="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-orange px-5 text-xs font-black text-white shadow-lg shadow-orange/20 transition hover:bg-orange/95"
            >
    ...
    <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        @foreach($metricCards as $metric)
            <x-card variant="default" class="p-3.5">
                <div class="flex items-center gap-2.5">
                    <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg {{ $metric['box'] }}">
                        {!! $metric['icon'] !!}
                    </span>
                    <span class="min-w-0">
                        <span class="block text-[9px] font-black uppercase leading-tight tracking-wider text-muted">{{ $metric['label'] }}</span>
                        <span class="mt-1 block break-words text-base font-black leading-tight tracking-tight text-ink">{{ $metric['value'] }}</span>
                    </span>
                </div>
            </x-card>
        @endforeach
    ...
    <div class="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        <x-card variant="default" class="p-4">
            <div class="flex flex-col gap-3 border-b border-border pb-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-black text-ink">Live Orders</h2>
                    </div>
                    <p class="mt-0.5 text-xs font-semibold text-muted"><span x-text="orders.length"></span> active orders</p>
                </div>
                <a href="{{ route('dashboard.orders.index') }}" class="inline-flex items-center gap-1.5 text-xs font-black text-orange hover:underline">
            ...
            <div class="mt-3 flex max-w-full items-center gap-1 overflow-x-auto rounded-lg border border-border bg-card-tint p-0.5">
                <button type="button" @click="activeTab = 'all'" :class="tabClass('all')" class="shrink-0 rounded-md px-3 py-1.5 text-[11px] font-black transition">All (<span x-text="orders.length"></span>)</button>
                <button type="button" @click="activeTab = 'preparing'" :class="tabClass('preparing')" class="shrink-0 rounded-md px-3 py-1.5 text-[11px] font-black transition">Preparing (<span x-text="countStatus('preparing')"></span>)</button>
                <button type="button" @click="activeTab = 'ready'" :class="tabClass('ready')" class="shrink-0 rounded-md px-3 py-1.5 text-[11px] font-black transition">Ready (<span x-text="countStatus('ready')"></span>)</button>
            </div>

            <div class="mt-3 overflow-hidden rounded-lg border border-border">
                <div class="overflow-x-auto">
                    <div class="min-w-[720px]">
                        <div class="grid grid-cols-[84px_84px_minmax(200px,1fr)_132px_118px_48px] gap-3 border-b border-border bg-card-tint px-3.5 py-2.5 text-[10px] font-black uppercase tracking-wider text-muted">
                            <span>Order</span>
                            <span>Time</span>
                            <span>Guest</span>
                            <span>Payment</span>
                        ...
                        <div class="divide-y divide-border/70 bg-card">
                            <template x-for="order in filteredOrders" :key="order.id">
                                <div
                                    ...
                                    class="grid min-h-[52px] cursor-pointer grid-cols-[84px_84px_minmax(200px,1fr)_132px_118px_48px] items-center gap-3 px-3.5 py-1.5 text-left text-[13px] transition hover:bg-card-tint/70"
                                >
                                    ...
                                    <span class="min-w-0">
                                        <span class="block truncate text-[13px] font-black text-ink" x-text="order.amount"></span>
                                        <span class="mt-0.5 block truncate text-[10px] font-black uppercase" :class="paymentClass(order)" x-text="order.paymentLabel"></span>
                                    </span>
                                    <span class="inline-flex w-fit items-center rounded-md px-2.5 py-1 text-[9px] font-black uppercase tracking-wider" :class="statusClass(order.status)" x-text="order.statusLabel"></span>
                                    <button
                                        type="button"
                                        @click.stop="openOrderDetails(order)"
                                        class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-border bg-card text-muted transition hover:border-orange/30 hover:text-orange"
                ...
                <div x-show="filteredOrders.length === 0" class="border-t border-border bg-card px-4 py-10 text-center">
            ...
            <div class="mt-3 flex items-center justify-between text-[11px] font-semibold text-muted">
                <span x-text="filteredOrders.length ? `Showing 1 to ${filteredOrders.length} of ${orders.length} orders` : 'No orders to show'"></span>
                <div class="flex items-center gap-2">
                    <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-border bg-card text-muted">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m15 18-6-6 6-6" />
                        </svg>
                    </button>
                    <span class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-orange text-xs font-black text-white">1</span>
                    <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-border bg-card text-muted">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.4">
            ...
            <x-card variant="default" class="p-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-black text-ink">Order Status Overview</h2>
                    </div>
                    <span class="rounded-lg border border-border bg-card px-2.5 py-1 text-[11px] font-black text-muted">Today</span>
                </div>
                        ...
                        <div>
                            <div class="mb-1.5 flex items-center justify-between gap-3 text-xs font-black">
                                <span class="flex min-w-0 items-center gap-2.5 text-muted">
                                    <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg {{ $tone['dot'] }}">
                            ...
                            <div class="h-1.5 overflow-hidden rounded-full bg-card-tint">
            ...
            <x-card variant="default" class="p-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-black text-ink">Top Selling Items</h2>
                    </div>
                    <a href="/reports" class="text-xs font-black text-orange hover:underline">View report</a>
                </div>
                    ...
                    @forelse($topItems as $index => $item)
                        <div class="flex items-center justify-between gap-3">
                            <div class="flex min-w-0 items-center gap-2.5">
                                <span class="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-orange/20 via-warning/20 to-teal/20 text-xs font-black text-orange">
                                    {{ $index + 1 }}
                                </span>
                                <span class="min-w-0">
                                    <span class="block truncate text-sm font-black text-ink">{{ $item['name'] }}</span>
                                    <span class="mt-0.5 block text-xs font-semibold text-muted">{{ $item['quantity'] }} sold</span>
                                </span>
                            </div>
                            <span class="shrink-0 text-xs font-black text-ink">Rs. {{ number_format($item['revenue'], 2) }}</span>
                        </div>
                    @empty
                        <div class="rounded-lg border border-dashed border-border bg-card-tint px-4 py-6 text-center">
